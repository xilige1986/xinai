...
export async function generateMetadata({ params }: CategoryMetadataProps): Promise<Metadata> {
  ...
  const title = `${category.name}类AI工具 - 精选${category.name}人工智能应用`;
  const description = category.description || `发现最佳${category.name}类AI工具，精选${category.name}领域的人工智能应用，助你提升工作效率。`;
  const keywords = `${category.name} AI工具,${category.name}人工智能,${category.name}应用,${category.name}软件,AI${category.name}`;
